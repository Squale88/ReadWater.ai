...
Per cell, all artifacts now live under
``data/areas/<area>/masks/water/`` (resolved through ``readwater.storage``):
...
Cell registry: cell ids and centers are read from the area's manifest via
``readwater.areas.Area``; the legacy ``scripts/_cells.py`` dependency is
gone. Any cell present in ``Area("rookery_bay_v2").cell_ids()`` is valid.
...
def fetch_styled_water(
    ...
    storage.atomic_write_bytes(out_path, resp.content)
    return out_path
...
def _perim_connected_at_zoom(
    ...
    """Fetch a wider styled tile at ``zoom`` to ``styled_path`` + return
    (water_mask, perim_connected_mask, bbox)."""
    fetch_styled_water(center, zoom, styled_path)
    wide_mask = water_mask_from_styled(styled_path)
...
def gulf_connected_mask_for_cell(
    ...
    The wide z14 / z13 styled PNGs are written through the storage
    helpers (``water_z14_wide_styled_path`` / ``water_z13_isolation_styled_path``)
    so downstream consumers (e.g. the drain detector's edge revalidation)
    can re-read them without rebuilding.
    """
    # The legacy code allowed an arbitrary --wide-zoom; the storage helper
    # only knows the canonical filename for z14. For non-canonical zooms we
    # fall back to a sibling filename in the same dir so the output layout
    # stays consistent.
    if wide_zoom == 14:
        detail_path = storage.water_z14_wide_styled_path(area_id, cell_id)
    else:
        detail_path = (
            storage.water_masks_dir(area_id)
            / f"{cell_id}_wide_z{wide_zoom}_styled.png"
        )
    detail_water, detail_perim, detail_bbox = _perim_connected_at_zoom(
        center, wide_zoom, detail_path, bridge_dilate_iters,
    )
    ...
    if isolation_zoom == 13:
        iso_path = storage.water_z13_isolation_styled_path(area_id, cell_id)
    else:
        iso_path = (
            storage.water_masks_dir(area_id)
            / f"{cell_id}_wide_z{isolation_zoom}_styled.png"
        )
    iso_water, iso_perim, iso_bbox = _perim_connected_at_zoom(
        center, isolation_zoom, iso_path, bridge_dilate_iters,
    )
...
def naip_aggressive_water_mask_aligned(
    ...
    """
    # TODO: NAIP TIFs still live in the legacy sibling directory
    # ``data/areas/<area>_naip``. Consolidating them under
    # ``area_root() / "naip" / ...`` is deferred to a future PR — they're
    # geotiff data with separate concerns and the fetch script (which
    # writes them) hasn't been migrated yet.
    naip_dir = storage.data_root() / "areas" / "rookery_bay_v2_naip"
...
def make_overlay(
    ...
    storage.atomic_write_bytes(out_path, buf.getvalue())
    return out_path
...
def run_one(
    ...
    styled_png = storage.water_styled_z16_path(area_id, cell_id)
    ...
    if connectivity_filter:
        gulf_connected = gulf_connected_mask_for_cell(
            area_id, cell_id, center, cell_bbox, google_mask.shape,
    ...
    mask_png = storage.water_mask_path(area_id, cell_id)
    buf = io.BytesIO()
    Image.fromarray((final_mask.astype(np.uint8) * 255), mode="L").save(
        buf, format="PNG",
    )
    storage.atomic_write_bytes(mask_png, buf.getvalue())

    # Use the existing satellite tile as the overlay base so the user
    # can put it side-by-side with the NAIP overlay.
    base_satellite = storage.z16_image_path(area_id, cell_id)
    overlay_png = storage.water_mask_overlay_path(area_id, cell_id)
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        action="append",
        default=[],
        help="Cell id like root-2-9. Pass 'all' for every cell in the area, "
             "or repeat the flag for multiple cells.",
    )
    ...
    print(f"\nArtifacts in: {storage.water_masks_dir(args.area)}")
    return 0
